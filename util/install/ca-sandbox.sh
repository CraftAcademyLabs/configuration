...
EXTRA_VARS="$EXTRA_VARS -e@/tmp/server-vars.yml"
...
##
cd /var/tmp
git clone https://github.com/CraftAcademyLabs/configuration
cd configuration
...
##
cd /var/tmp/configuration/playbooks && sudo -E ansible-playbook -c local ./ca_edx_sandbox.yml -i "localhost," $EXTRA_VARS "$@"
