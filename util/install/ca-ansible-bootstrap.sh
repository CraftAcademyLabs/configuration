...
if [[ -z "${CONFIGURATION_REPO}" ]]; then
  CONFIGURATION_REPO="https://github.com/CraftAcademyLabs/configuration.git"
fi

if [[ -z "${CONFIGURATION_VERSION}" ]]; then
  CONFIGURATION_VERSION="ca/ginkgo.2"
fi
...
apt install -y python2.7 python2.7-dev python-pip python-apt python-yaml python-jinja2 build-essential sudo git-core libmysqlclient-dev libffi-dev libssl-dev
...
if [[ "xenial" = "${SHORT_DIST}" ]]; then
    #apt install -y python2.7 python2.7-dev python-pip python-apt python-yaml python-jinja2
    pip install --upgrade pip
    pip install --upgrade pip=="${PIP_VERSION}"
    #apt install -y build-essential sudo git-core libmysqlclient-dev
else
    #apt install -y python2.7 python2.7-dev python-pip python-apt python-yaml python-jinja2 build-essential sudo git-core libmysqlclient-dev
    pip install --upgrade pip=="${PIP_VERSION}"
fi
...
if [[ "true" == "${RUN_ANSIBLE}" ]]; then
    ...
    # Switch to master here since the upcoming playbook reverts to the official edx/configuration repo
